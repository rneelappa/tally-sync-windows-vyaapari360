...
async function testTrialBalanceLedgers() {
  const TALLY_URL = 'https://e34014bc0666.ngrok-free.app';
  
  console.log('🔄 Testing Trial Balance for ledger data...');
...
<ENVELOPE>
  ...
  try {
    ...
    const ledgers = extractMasterData(parsedData, 'LEDGER');
    console.log(`🎯 Final result: ${ledgers.length} ledgers extracted from Trial Balance`);
    
    if (ledgers.length > 0) {
      console.log('📋 Sample ledgers:');
      ledgers.slice(0, 5).forEach((ledger, index) => {
        console.log(`${index + 1}. ${ledger.NAME} (GUID: ${ledger.GUID})`);
        if (ledger.OPENINGBALANCE !== undefined) console.log(`   Opening: ${ledger.OPENINGBALANCE}`);
        if (ledger.CLOSINGBALANCE !== undefined) console.log(`   Closing: ${ledger.CLOSINGBALANCE}`);
      });
    }
